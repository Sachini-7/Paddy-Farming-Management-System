...
router.route("/loginCus").post(async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET);
    return res.status(200).json({ message: `${username} login successfully`, nic: user.nic, isHR: false, token });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal Server Error", error: error.message });
  }
...
router.route("/checkNIC/:nic").get(async (req, res) => {
  try {
    ...
    // Access the "requestforms" collection directly using Mongoose's connection
    ...
    // Use the $lookup stage to perform a left outer join between "requestforms" and "harvest"
    // on the "farmerId" field from "requestforms" and the "NIC" field from "harvest"
    const mergedData = await requestFormsCollection.aggregate([
      {
        $match: { farmerId: nic }
      },
      {
        $lookup: {
          from: "harvests",
          let: { farmerId: "$farmerId" }, // Define a variable to hold the "farmerId" value
          pipeline: [
            {
              $match: {
                $expr: {
                  $eq: ["$NIC", "$$farmerId"] // Compare "NIC" from "harvest" with "farmerId" from "requestforms"
                }
              }
            }
          ],
    ...
    if (mergedData.length > 0) {
      ...
      // If no data found, you can return an appropriate message
...
router.route("/checkNIC/:nic").get(async (req, res) => {
  try {
    ...
    if (requestForms.length > 0 || harvestData.length > 0 || distributionsData.length > 0 || seedReqsData.length > 0) {
      // Combine data from both tables into a single array
      const mergedData = [...requestForms, ...harvestData, ...distributionsData, ...seedReqsData];
      res.status(200).json(mergedData);
    } else {
