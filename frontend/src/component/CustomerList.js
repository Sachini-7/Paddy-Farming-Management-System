...
const handleUpdateCustomer = (nic) => {
  ...
  <input
        ...
        maxLength={13}
      />
  <table className="customer-list-table" ref={tableRef}>
    <thead>
      <tr>
        <th>First Name</th>
        <th>Last Name</th>
    ...
    <tbody>
      {customers.map((customer, index) => (
         ...
         className={highlightedCustomerIndex === index ? 'highlighted-row' : ''}
        >
          ...
          <td>
            <button className="dltbtn" onClick={() => confirmDelete(customer.nic)}>Delete</button>
          </td>
          <td>
          <button className="update-btn" onClick={() => handleUpdateCustomer(customer.nic)}>Update</button>
          </td>
        </tr>
      ))}
    </tbody>
  </table>
  <button className="download-button" onClick={downloadCustomerList}>
    Download from here
  </button>
</div>
  );
